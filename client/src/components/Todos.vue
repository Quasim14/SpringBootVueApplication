<template>
  <div>
    ...
    <h1 class="email">Connecter en tant que : {{userEmail}}</h1>
      ...
      <div v-else>
        ...
        <footer class="footer" v-show="todos.length" v-cloak>
          <span class="todo-count">
            <strong>{{ remaining }}</strong> {{ remaining | pluralize }} restant
          </span>
          <ul class="filters">
            <li><a href="#/all" @click="setVisibility('all')" :class="{ selected: visibility == 'all' }">Toutes</a></li>
            <li><a href="#/active" @click="setVisibility('active')" :class="{ selected: visibility == 'active' }">Active</a></li>
            <li><a href="#/completed" @click="setVisibility('completed')" :class="{ selected: visibility == 'completées' }">Completed</a></li>
          </ul>
  ...
  const Todos = {
    ...
    computed: {
      ...
      inputPlaceholder: function () {
        return this.activeUser ? this.activeUser.given_name + ', que devez-vous faire ?' : 'que devez-vous faire?'
      }
    },
